...
class PytorchTrainingAndTest:
    ...
    def run_model_kfold(self, exp_num, model, model_name, database_name, train, test, learning_rate, num_epochs, num_class=2):
        '''
          function to train the model using pytorch lightning
          params:
            - exp_num: num of experiments to run a model
        ...
        #define callback for earlystopping
        early_stop_callback = EarlyStopping(monitor='val_loss', min_delta=0.01, patience=5, verbose=True, mode='max')
    ...
    def save_metrics_to_figure(self, metrics, model_name, database_name):
        ...
        plt.tight_layout()
        fig_acc.savefig(os.path.join("../", "metrics", "figures", "{}-{}-{}.png".format('auc_metrics',model_name, database_name)))
